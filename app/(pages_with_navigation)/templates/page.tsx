  ...
  <div className="bg-base-200 border border-base-300 rounded-xl overflow-hidden transition-all duration-300 cursor-pointer flex flex-col hover:shadow-lg hover:-translate-y-1">
    <div
      ...
      style={{
        ...
        color:
          item.project?.canvas?.backgroundColor?.includes('fff') ||
          item.project?.canvas?.backgroundColor?.includes('f5f')
            ? '#333'
            : 'oklch(26.71% 0.049 252.525)',
      }}
    >
      <div>
        <div className="text-base font-bold mb-1">{item.name}</div>
...
const Section: React.FC<{
  ...
  onOpen?: (item: Template) => void;
}> = ({ title, items, onUse, onOpen }) => {
