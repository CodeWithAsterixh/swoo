...
import {
  encryptData,
  decryptData,
} from "../../../../business-card-editor/lib/encryption";
...
const CreatePage: React.FC = () => {
  ...
  const [saveMode, setSaveMode] = useState<"remote" | "local" | "both" | null>(
    null
  );
  ...
  const handleCreate = async (mode: "remote" | "local" | "both") => {
    ...
    try {
      ...
      if (
        result.remote &&
        body.recommended?.action === "redirect" &&
        body.recommended.url
      ) {
    ...
    <main className="min-h-screen bg-base-100 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl bg-base-200 border border-base-300 rounded-2xl p-8">
        ...
        <p className="text-base text-base-content/70 mb-8 leading-relaxed">
          Choose how you want to save your new business card design. You can
        ...
        {steps && steps.length > 0 && (
          <div className="bg-base-100 border border-base-300 rounded-lg p-4 mb-5">
            ...
            <div className="flex flex-col gap-2">
              {steps.map((s) => {
                const statusErrorColor =
                  s.status === "error"
                    ? "rgba(255, 107, 107, 0.2)"
                    : "oklch(88.272% 0.011 223.975)";
                const statusColor =
                  s.status === "success"
                    ? "rgba(76, 175, 80, 0.2)"
                    : statusErrorColor;
                return (
                  ...
                  >
                    <div
                      className="w-5 h-5 min-w-5 rounded-full flex items-center justify-center text-xs"
                      style={{
                        background:
                          s.status === "running"
                            ? "oklch(80% 0.114 19.571 / 0.2)"
                            : statusColor,
                      }}
                    >
                      {s.status === "running" && "⏳"}
        ...
        {!saveMode && !loading && (
          <div className="p-4 bg-accent/10 border border-accent/30 rounded-lg text-accent text-xs leading-relaxed">
            <strong>💡 Tip:</strong> Save Both keeps your design synced between
            cloud and offline storage. You&lsquo;ll always have a backup.
          </div>
        )}
      </div>
    </main>
